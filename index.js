...
try:
    ...
    content = '[첫번째 상품]: ' + str(list1) + '[두번째 상품]: ' + str(list2)
    print('2번 선택')
...
while True:
    time.sleep(0.1) # infinity loop를 방지하기 위해
    `;
    ...
    PythonShell.run("/exec.py", options, function (err, results) {
      ...
      if (err) {
        ...
        fs.readFile("./output.out", "utf8", function (err, data) {
          ...
          var html = template.html("실행 결과", sanitizedData, eyescape);
          response.send(html);
        });
      }
    });
...
app.post("/exit_process", (request, response, next) => {
  ...
  exec("taskkill /F /IM python.exe /T", function (err, stdout, stderr) {
    if (err) {
      console.error(err);
    }
    console.log(stdout);
